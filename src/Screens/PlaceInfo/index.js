	...
	return (
		<div>
			...
			<section className={styles.details}>
				<span className={styles['details--card']}>
					...
					<p>
						...
						{location?.more_info_link ? (
							<a href={'https://en.wikipedia.org/wiki/Nike_Art_Gallery'} target="_blank" rel="noreferrer">
								read more here
							</a>
						) : null}
					</p>
				...
				<div>
					<span className={styles['details--card']}>
						<BuildingOfficeIcon className={`h-6 w-6 text-gray-500 ${styles.icon}`} />
						<p>Hours</p>
						<h3>
			...
			{location?.coordinate_x && location?.coordinate_y ? <Map position={[location.coordinate_x, location.coordinate_y]} /> : null}
		</div>
	);
};
